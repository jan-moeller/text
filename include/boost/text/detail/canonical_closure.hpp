...
namespace boost { namespace text { namespace detail {
    ...
    template<typename OutIter>
    OutIter recurse_and_output_canonical_closure_segment(
        canonical_closure_subsegments_t & subsegments, OutIter out) noexcept
    {
        auto total = 1;
        for (auto i = 0, end = (int)subsegments.size(); i != end; ++i) {
            if (!i) {
            ...
            for (auto j = 0, j_end = (int)subsegments[i].size(); j != j_end;
                 ++j) {
    ...
    segment_canonical_closure(CPIter first, CPIter last, OutIter out) noexcept
    {
        ...
        canonical_closure_buffer_t comps;
        {
            ...
            std::transform(
                ...
                [](std::pair<uint32_t, uint32_t> pair) { return pair.second; });
        }
        ...
        for (auto comp : comps) {
            ...
            subsegments.back().push_back(canonical_closure_string_t(1, comp));
            ...
            for (auto input_it = std::next(first), end = last; input_it != end;
                 ++input_it) {
